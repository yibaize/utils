 ...
 * @作者： big
 * @创建时间： 2018/6/5
 * @文件描述：反射excel生成java对象工具 ：注 这位工具依赖于 poi-ooxml 和 poi 两个Java包
 */
public class ExcelUtils {
    ...
    private static void serializerObj(List<Map<String, String>> objs, String clazzName) {
        ...
        for (int i = 0; i < objs.size(); i++) {
            ...
            for (Map.Entry<String,String> e:objs.get(i).entrySet()){
                try {
                    ...
                    if(valueObj == null)
                        System.err.println("excel表在反射初始化"+clazz+"类时出现异常,没有"+field+"的"+valueObj+"这个类型");
        ...
        if(staticConfigMessage == null)
            System.err.println("获取静态初始化类"+staticConfigMessage+"时出现异常取不到这个类");
